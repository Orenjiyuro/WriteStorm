...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the Codex feasibility authority. The admitted wording is `Conditional Go — Windows feasibility verified; macOS packaged runtime deferred-by-user.` `config/block13-ai-gate-v1.json` is the single versioned application Gate projection of that verdict; Main and renderer consume its strict frozen projection rather than duplicating Gate values. Atomic R8b certification from clean pushed HEAD `834ba0b` passed 869 unit tests, 290 integration tests, packaged E2E 15/15, package verification 7/7, conditional development admission, all four lifecycle scenarios, the packaged-only SDK turn and unified verification. Certification `b6a-834ba0b6-57d262ae` binds the exact Windows artifact and seven sanitized records. Total-thread review subsequently passed Task 13.1; Task 13.2 passed supply-chain/freshness review; Tasks 13.3–13.12 and the D108–D114 safety, diagnostic, product-packaging, no-global-Git and explicit Settings-check decisions are implemented as recorded in `V1-BLOCK-13-STATUS.md`. Task 13.11 evidence at product runtime HEAD `83e6e03` and the later Task 13.5 no-global-Git record at HEAD `2538722` both passed their recorded Windows development-machine gates. Task 13.12 changes the production-protocol and probe-artifact fingerprints, so both records are now historical and runtime compatibility fails closed as stale pending separately authorized recertification. Settings exposes only explicit `ai:check-connection`, displays Gate, compatibility and ephemeral runtime/auth observation separately, performs no startup/navigation/Library-open check and keeps every AI action disabled. No Task 13.12 real SDK/auth/network turn ran. The application auth observation remains unknown; expired, permission-denied, rate-limit and network classifications remain unverified and have no Codex production producer. No production AI Job, SQLite persistence, checkpoint, renderer generation action or resumable result exists. Clean-machine, signing, Defender, proxy, enterprise certificate, firewall, offline, telemetry and macOS boundaries remain unverified or deferred. This is not full Go, cross-platform compatibility or release readiness.
