...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the Codex feasibility authority. The admitted wording is `Conditional Go — Windows feasibility verified; macOS packaged runtime deferred-by-user.` Atomic R8b certification from clean pushed HEAD `834ba0b` passed 869 unit tests, 290 integration tests, packaged E2E 15/15, package verification 7/7, conditional development admission, all four lifecycle scenarios, the packaged-only SDK turn and unified verification. Certification `b6a-834ba0b6-57d262ae` binds the exact Windows artifact and seven sanitized records. Total-thread review subsequently passed Task 13.1; Task 13.2 passed supply-chain/freshness review; Tasks 13.3–13.10 and the D108–D110 safety/diagnostic remediations are implemented as recorded in `V1-BLOCK-13-STATUS.md`, including the three-layer compatibility record rebound to clean runtime HEAD `c6ee108`. Task 13.11 is not started. The application auth observation remains unknown until a future explicit connection check; expired, permission-denied, rate-limit and network classifications remain unverified and have no Codex production producer. Strict non-transforming structured-output validation, provider-private raw stream budgets, exact reported-or-unknown usage, a bounded in-memory diagnostic log, a bounded in-memory attempt/event candidate state machine and a quarantining lifecycle/cleanup boundary are admitted, but no production AI request or real SDK turn is connected to that lifecycle path and adapter capabilities remain false. No candidate or diagnostic record is persisted or resumable. This is not full Go, cross-platform compatibility or release readiness.
