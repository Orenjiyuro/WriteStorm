...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the current Codex feasibility authority. Its current status is `pending recertification — post-2389170 remediation changed the certified boundary; macOS deferred-by-user`. Clean integration HEAD `2389170` remains a valid historical Windows-only conditional-Go checkpoint, but later remediation changed runtime, certification configuration, admission, lineage, package guards and operational manifest inputs. Its seven records and artifact cannot certify the current candidate. A fresh clean pushed HEAD must complete the atomic Windows certification pipeline before the candidate verdict can return to `conditional Go — Windows-only feasibility verified; macOS deferred-by-user`. Task 13.1 remains blocked and Task 13.2 is not authorized. This is not full Go, cross-platform compatibility or release readiness; historical Block 7/8 unexecuted facts also remain historical facts.
