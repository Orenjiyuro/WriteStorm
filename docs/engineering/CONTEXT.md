...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the current Codex feasibility authority. Its current status is `conditional Go — Windows-only feasibility verified; macOS deferred-by-user`. Atomic R8b certification from clean pushed HEAD `834ba0b` passed 869 unit tests, 290 integration tests, packaged E2E 15/15, package verification 7/7, conditional development admission, all four lifecycle scenarios, the packaged-only SDK turn and unified verification. Certification `b6a-834ba0b6-57d262ae` binds the exact Windows artifact and seven sanitized records. This is not full Go, cross-platform compatibility or release readiness. Task 13.1 remains blocked pending total-thread review and Task 13.2 is not authorized; historical Block 7/8 unexecuted facts and the `2389170` checkpoint remain historical facts.
...
The packaged-only SDK turn from that historical integration HEAD passed with authenticated structured success and cleanup acknowledgement. Its `packaged_sdk` record binds the exact full-check artifact hash, lockfile/runtime boundary and ten static inputs. The two static privacy/protocol assertions retain their own provenance. Later post-`2389170` remediation expires application of this evidence chain to the current candidate; fresh Windows certification remains required. Task 13.1 remains blocked, Task 13.2 remains unauthorized, and macOS packaged runtime remains `deferred-by-user`.

Fresh atomic R8b certification from clean pushed HEAD `834ba0b6dfef20c223181af1bf33cfd6ebf703f2` closes that post-`2389170` expiry. Development was admitted with the four documented message-only Git/auth limitations, lifecycle and packaged gates were admitted without blockers, and unified verification published immutable bundle `out/6a/b6a-834ba0b6-57d262ae`. The artifact SHA-256 is `4b06acd811473831369cc60d4b012bd2eca2c2805c005fa49ca11c50dc631966`; the committed bundle manifest binds all seven current evidence records and the exact artifact manifest. Task 13.1 still awaits total-thread review, Task 13.2 remains unauthorized, and macOS remains `deferred-by-user`.
