...
The machine-readable disposition authority is `docs/engineering/block6a-feasibility-harness-exit-plan.json`. It classifies every current file under `src/main/codex-feasibility` exactly once. Nothing in that directory is a production `AiExecutionPort` or `CodexProviderAdapter`, and Task 13/17 may not import the directory as a shortcut.
...
Before Task 13.2 authorization, a mechanical guard must prove production code imports no feasibility source. Before production adapter composition, the new production contracts and tests must exist independently. Before the first product AI execution, `src/main/codex-feasibility` must be absent and any retained Block 6A fixture, admission, lineage and certification tooling must be outside the product source and default package graph. Before Task 17 release readiness, that replacement certification path must pass against the final artifact. These gates define future work only; they do not authorize Task 13.2, implement a Job adapter or change macOS `deferred-by-user` status.
