...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the Codex feasibility authority. The admitted wording remains `Conditional Go — Windows feasibility verified; macOS packaged runtime deferred-by-user.` `config/block13-ai-gate-v1.json` is the single versioned application Gate projection of that verdict; Main and renderer consume its strict frozen projection rather than duplicating Gate values. Atomic R8b certification from clean pushed HEAD `834ba0b` remains immutable feasibility evidence. Tasks 13.3–13.13 have implementation, and D117–D120 keep final packaged admission dependent on separately reviewed provenance. The temporary 6A implementation has exited `src/main` and is retained only as a certification fixture under `tests/certification/block6a/runtime`. Historical Windows probes at clean runtime HEAD `df7e9d20d1e2e24e04aafa62464092a39c3a8121` passed product success/cancel/timeout and all 17 no-global-Git assertions under compatibility fingerprint `c288a454be6e4546c321b16ec3fa6c6ac8a6f8be644fd8551d6f5ca566c374d8`. The new canonical compatibility leaf, explicit package command and Settings natural-path runner change the production-protocol, supply-chain and probe-artifact layers, so those records and their exact-byte receipt are stale for the current candidate. `AiCompatibilityAssessment` now has one import-free Main-owned declaration. The new runner is outside `npm run check`, starts normal product Main, clicks the real Settings entry and button, and requires pre-click no-external-connection/no-AI-process observations plus visible auth/`observedAt`, cleanup and residual assertions. It has not been run at the current implementation HEAD. Until current product, no-global-Git and Settings natural-path evidence is separately authorized and accepted, Block 13 remains reopened, compatibility is stale and auth/runtime observation is unknown. macOS can load the non-AI Forge configuration but its Codex runtime stays deferred/blocked and no Codex package is bundled there. Settings performs no startup/navigation/Library-open check and keeps every AI action disabled. Product sample preview and module analysis are blocked by the unadmitted Block 17 production Job/prompt runtime, not by an unpassed Codex Gate. No production AI Job, SQLite persistence, checkpoint, renderer generation action or resumable result exists. Clean-machine, signing, Defender, proxy, enterprise certificate, firewall, offline, telemetry and macOS runtime boundaries remain unverified or deferred.
...
The historical post-remediation R8a development run from clean HEAD `e1db3d2` produced complete, lineage-verified five-scenario capability and two-scenario outputSchema records, but every SDK scenario ended `runtime_failed / unverified`. Admission therefore exited non-zero at that checkpoint: current auth did not re-establish a real success, stable Git/login classifications remained unavailable, and real structured output was not re-established. The evidence is retained without raw errors; later R8b certification superseded this checkpoint for current feasibility.
...
R8a4 identified why that early invalid-schema rejection could not satisfy the exact pinned-version local guard: Vite's CJS build transformed `createRequire(import.meta.url)` into an invalid `createRequire({}.url)`, and the SDK root exposes an import-only entry that cannot be located with `require.resolve`. The utility now anchors `createRequire` to CJS `__filename`, finds the nearest verified WriteStorm package root and admits only an exact package-name manifest under its local `node_modules`; the repository `package.json` is the non-CJS test fallback. A focused test builds the real utility CJS entry, rejects the broken transform and verifies local version `0.144.6` resolution without reading an SDK error. Future lineage requires the R8a4 static manifest as a ninth ordered input. This runtime and lineage change leaves the `c7fa672` records as historical blocked evidence and requires a fresh development run from the next clean committed HEAD; no SDK turn ran during R8a4.

The fresh development run from clean HEAD `964979c` accepted complete, sanitized nine-input evidence but did not admit recertification. Current ChatGPT-managed auth completed a real SDK success, valid-minimal outputSchema passed strict validation, and the fixed invalid schema was rejected by the pinned local guard. The four isolated-auth scenarios did not provide stable structured Git/auth facts: three reached WriteStorm's local turn deadline and the non-Git check rejected earlier only as `sdk_unstructured`. Admission therefore returned `git_auth_structured_classification_unavailable`, `local_sdk_turn_deadline_exceeded` and `sdk_unstructured_runtime_failure`, exited non-zero and blocked lifecycle/package execution. Windows status remains pending; this run does not restore the historical conditional verdict.

R8a5 replaces that impossible structured-error prerequisite with a layered conditional development gate. Hard requirements are current-auth Git success, valid structured output, the pinned invalid-schema guard, the current-auth non-Git behavioral differential, privacy/process boundaries and lineage. The differential uses the same current auth and non-Git workspace with Git checking enabled and then with `skipGitRepoCheck=true`; the checked case must fail only as generic unavailable while the bypass case must succeed. SDK 0.144.6's message-only Git/auth failures and isolated-empty-auth deadlines remain machine-readable conditional limitations rather than hard blockers. Every unknown SDK/CLI failure is `runtime_failed / unverified` with only `SDK_RUNTIME_UNAVAILABLE`; it can never become `AUTH_FAILED` or `GIT_REQUIRED`. Future runtime lineage requires ten ordered static inputs. The `964979c` records remain a valid blocked checkpoint for their nine-input boundary, but a fresh seven-scenario run from the next clean committed HEAD is required before lifecycle.

Historical integration checkpoint `2389170` includes the authorized Library-session cache fix `602500f`. Its fresh `npm run check` was fully green: typecheck, 831 unit tests, 290 integration tests, packaged E2E 15/15 and package boundary 7/7. The resulting artifact is the one bound by that checkpoint's packaged SDK evidence.

At that same historical integration HEAD, development recertification passed as `admitted_with_conditions` with no blocker. Current-auth Git, the non-Git bypass differential, valid outputSchema and the pinned invalid-schema guard passed; four known Git/auth diagnostic limitations remained. Lifecycle then passed app-timeout, explicit-cancel, window-close and app-quit with abort observation, SDK settlement, cleanup acknowledgement, utility exit and attributed zero-residual checks.

The packaged-only SDK turn from that historical integration HEAD passed with authenticated structured success and cleanup acknowledgement. Its `packaged_sdk` record binds the exact full-check artifact hash, lockfile/runtime boundary and ten static inputs. The two static privacy/protocol assertions retain their own provenance. Later post-`2389170` remediation expired application of this evidence chain to the then-current candidate; R8b subsequently supplied fresh Windows certification. At the `2389170` checkpoint Task 13.1 remained blocked and Task 13.2 remained unauthorized.

Fresh atomic R8b certification from clean pushed HEAD `834ba0b6dfef20c223181af1bf33cfd6ebf703f2` closes that post-`2389170` expiry. Development was admitted with the four documented message-only Git/auth limitations, lifecycle and packaged gates were admitted without blockers, and unified verification published immutable bundle `out/6a/b6a-834ba0b6-57d262ae`. The artifact SHA-256 is `4b06acd811473831369cc60d4b012bd2eca2c2805c005fa49ca11c50dc631966`; the committed bundle manifest binds all seven current evidence records and the exact artifact manifest. Total-thread review later accepted this evidence for Task 13.1 and separately authorized Task 13.2. macOS remains `deferred-by-user`.
...
D065 established the Task 12.10 natural-path blocked shell. After Block 13's Windows conditional Gate was accepted, the active sample-preview blockers are `prompt_template_instance_unavailable` and `sample_preview_runtime_not_admitted`; the obsolete `codex_sdk_gate_required` product claim is removed. The page still states that publication requires a passed sample preview. The Settings route has no action callback or privileged bridge. No sample fixture, Prompt body, Job, SDK, provider or AI runtime is admitted here; Block 17 remains the real preview owner.
...
Task 9.7 adds three explicit disabled affordances to the selected module detail: Run analysis, Rerun module, and View rerun diff. Each native disabled button has its own visible, accessibility-linked reason. The active analysis and rerun blockers are the still-unadmitted Block 17 production AI Job/prompt runtime; diff is also blocked because no rerun candidate or diff implementation exists. The earlier Task 9 checkpoint's Codex-spike blocker was superseded by Block 13's Windows conditional Gate. These are presentation-only facts: there are no callbacks, mutations, IPC handlers, Jobs, candidate revisions, generated content, or hidden fallback AI path.
