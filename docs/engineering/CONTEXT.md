...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the Codex feasibility authority. The admitted wording is `Conditional Go — Windows feasibility verified; macOS packaged runtime deferred-by-user.` `config/block13-ai-gate-v1.json` is the single versioned application Gate projection of that verdict; Main and renderer consume its strict frozen projection rather than duplicating Gate values. Atomic R8b certification from clean pushed HEAD `834ba0b` passed 869 unit tests, 290 integration tests, packaged E2E 15/15, package verification 7/7, conditional development admission, all four lifecycle scenarios, the packaged-only SDK turn and unified verification. Certification `b6a-834ba0b6-57d262ae` binds the exact Windows artifact and seven sanitized records. Total-thread review subsequently passed Task 13.1; Task 13.2 passed supply-chain/freshness review; Tasks 13.3–13.13 and the D108–D116 safety, diagnostic, product-packaging, no-global-Git, explicit Settings-check and final integration decisions are implemented as recorded in `V1-BLOCK-13-STATUS.md`. Block 13 is complete only under that exact platform-limited Gate. Separately authorized Windows recertification at clean runtime HEAD `f4e6adc2d106946de39563da1da4ed986c4caed8` passed the real product success/cancel/timeout sessions and all 17 certification-artifact no-global-Git assertions under compatibility fingerprint `c70e0d116d0bcada33c381701faee7a90b6e28570eeff8be82020f50c10807d1`. A subsequent build therefore projects compatibility as fresh, but application auth observation still begins unknown and changes only after the user explicitly invokes `ai:check-connection`; default tests never invoke that real channel. Settings displays Gate, compatibility and ephemeral runtime/auth observation separately, performs no startup/navigation/Library-open check and keeps every AI action disabled. Expired, permission-denied, rate-limit and network classifications remain unverified and have no Codex production producer. No production AI Job, SQLite persistence, checkpoint, renderer generation action or resumable result exists. Clean-machine, signing, Defender, proxy, enterprise certificate, firewall, offline, telemetry and macOS boundaries remain unverified or deferred. This is not full Go, cross-platform compatibility or release readiness.
