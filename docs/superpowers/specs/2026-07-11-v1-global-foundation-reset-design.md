...
- 错误原因必须能决定具体恢复动作。
- 所有可识别错误必须穷尽映射到 `user_action`、`retry_later`、`restart_required` 或 `internal_failure`；禁止使用默认 `recoverable: true`。
- wire envelope 在 foundation reset 期间可暂时保留兼容的 `recoverable` 字段，但该布尔值必须由 recovery disposition 推导：仅 `user_action` 和 `retry_later` 为 `true`。
- Structure detection 已建立首个共享 taxonomy 使用点；foundation reset 的 Zod-first error contract 必须复用该词汇，而不是继续扩张单一 `STRUCTURE_ERROR` envelope。
...
```ts
export const sourceTextMetadataSchema = z
  .object({
...
```ts
expect(Object.keys(CONTRACT_REGISTRY).sort()).toEqual(
...
```ts
export const librarySessionSummarySchema = z
  .object({
