...
This document is the current authority for Task 6A.1 through Task 6A.8b and the subsequent remediation. Tasks 6A.1–6A.8b record the original spike and its historical Windows-only conditional verdict. R1–R7 subsequently changed admission, environment, protocol, supervision, process ownership, cleanup, error classification, assertion provenance and evidence lineage behavior. Those changes triggered the recorded expiry conditions. The fresh R8a development run from `e1db3d2454dc568ca591ae7c72d44b83d92e6723` retained valid sanitized evidence but did not pass recertification admission. R8a3 then changed the runtime result contract to attribute only the safe local failure boundary, so that blocked run remains historical evidence for its exact boundary and a new clean development run is required. The Windows verdict remains pending and blocked at the development gate.
