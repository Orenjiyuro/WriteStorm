...
Final verification requires content admission for the exact two development, four lifecycle and one packaged records, followed by `git merge-base --is-ancestor gitHeadAtRun finalHead`, exact current lock/runtime/artifact/input hashes, and a `git diff --name-only gitHeadAtRun finalHead` containing only Block 6A evidence JSON, the three current authority documents, or the two corresponding authority/lineage consistency tests. Any runtime, probe, admission, package, lock, Forge/Vite or unrelated test change invalidates the run and requires fresh execution. The unified verifier requires an explicit artifact root, rejects missing/extra/tampered evidence before emitting a verdict, and mechanically emits the fixed Windows-only conditional verdict only after all seven records pass both admission and lineage.
...
### Historical certified checkpoint: Windows-only conditional Go

R1–R7 changed the evidence admission contract, utility environment, typed protocol, unified utility-session orchestration, session/termination supervision, owned-process cleanup, error classification, assertion provenance and evidence lineage. Those expiry-condition changes invalidated application of the historical Task 6A.8b record to the repaired implementation. R8a5 then supplied fresh development, lifecycle and Windows x64 packaged evidence against clean committed run boundaries. At clean integration HEAD `2389170`, Windows implementation feasibility was verified under the recorded conditions.

The historical `2389170` decision was `conditional Go — Windows-only feasibility verified; macOS deferred-by-user`. It depended on existing ChatGPT-managed auth and retained generic-only handling for SDK/CLI failures without a stable structured signal. It was not a full Go, cross-platform claim or release-readiness claim. It did not erase the dated 6A.5–6A.8 evidence, automatically unblock Task 13.1 or authorize Task 13.2.

The historical static decision record is `docs/engineering/evidence/block6a-r8a5-windows-conditional-go-verdict.json`. It reconciles seven independently sourced `2389170` records without upgrading their provenance. `docs/engineering/evidence/block6a-remediation-pending-recertification.json` remains the earlier historical R1–R7 pending-status checkpoint.
...
`probe:codex:dev` ran from clean committed and pushed integration HEAD `2389170481d95e644c8663af25fd0a7f8cec5d7c`. The fixed input and expected-value hashes matched the runner before process-local injection. The runner returned `evidenceAccepted: true`, `admission: admitted_with_conditions`, `recertificationAdmitted: true`, no blockers and exactly four conditional limitations: `git_auth_structured_classification_unavailable`, `isolated_auth_local_turn_deadline_observed`, `isolated_auth_sdk_runtime_unavailable_observed` and `current_auth_non_git_check_failure_generic_only`.
...
The isolated diagnostic matrix remained conservative: three scenarios reached WriteStorm's local turn deadline and one rejected earlier only as `sdk_unstructured`; all four expose only `SDK_RUNTIME_UNAVAILABLE`. No result is upgraded to login, auth or Git business facts. The exact records are `docs/engineering/evidence/block6a-r8a5-windows-dev-capability-admitted-with-conditions.json` and `docs/engineering/evidence/block6a-r8a5-windows-dev-output-schema-admitted-with-conditions.json`. They bind clean integration run HEAD `2389170`, lockfile/runtime hashes and ten ordered inputs, and contain no synthetic value, response body, stdout/stderr, environment value, credential, auth file, process identity, executable path or raw SDK error.
...
Each record binds clean integration run HEAD `2389170`, the unchanged lockfile/runtime hashes and ten ordered static evidence inputs; `packagedArtifactSha256` remains null because this was not a packaged run. The records retain no synthetic value, response body, stdout/stderr, environment value, credential, auth file, PID, executable path, process list or raw SDK error. Fresh lifecycle success authorizes only the Windows packaged feasibility gate. It does not itself authorize Task 13.2; macOS remains `deferred-by-user`.

### R8a5 fresh Windows packaged result

The fresh `npm run check` from clean committed and pushed integration HEAD `2389170481d95e644c8663af25fd0a7f8cec5d7c` created the Windows x64 Electron Forge artifact and completed the package-boundary suite 7/7. Without rebuilding that artifact, `node scripts/run-block6a-probes.mjs packaged` then ran the packaged-only SDK turn from the same HEAD. The packaged runner returned `evidenceAccepted: true`, `admission: admitted`, `recertificationAdmitted: true`, no blockers and no conditional limitations.
...
The record binds integration run HEAD `2389170`, the unchanged lockfile/runtime-boundary hashes, ten ordered static evidence inputs and the concrete packaged artifact SHA-256. The artifact contains only the required Windows platform runtime under the enforced allowlist/unpack boundary; the package suite rejects another platform record in actual resources or a missing required Windows runtime file. The record contains no synthetic value, response body, stdout/stderr, environment value, credential, auth file, PID, executable path, process list or raw SDK error. Its two static boundary assertions retain `static_manifest` provenance instead of inheriting `packaged_sdk`.

At `2389170`, development, lifecycle and packaged gates supported the historical `conditional Go — Windows-only feasibility verified; macOS deferred-by-user` decision. That was a feasibility verdict only, and the later remediation expires its application to the current candidate. Task 13.1 remains blocked and Task 13.2 is not authorized. No direct CLI, app-server, GUI automation, API-key, local-model or alternate-provider fallback is authorized.

### Fresh integration full-check status

After explicit authorization, commit `602500f` was cherry-picked as integration commit `2389170`; it changes only `src/renderer/app/AppRouter.tsx` and `tests/unit/renderer-service-state.test.ts`. The fresh `npm run check` from that clean and pushed HEAD completed typecheck, all 165 unit files with 831/831 tests, all 35 integration files with 290/290 tests, packaged E2E with 15/15 tests, and the chained package-boundary suite with 7/7 tests. The previously failing Library-session natural path was green. This is the historical full-check result for the artifact used by the `2389170` packaged SDK evidence.
...
The current recertification source is the committed public, non-sensitive fixture `fixtures/block6a/codex-sdk-feasibility-v1.json`. It is independently available to every reviewer and records `fixtureId: block6a-codex-sdk-feasibility-v1`, owner `WriteStorm engineering authority`, its exact SHA-256 fingerprints and its rotation policy. Changing either value requires a new fixture ID, new pinned hashes, review approval and fresh Windows recertification. Historical runs that used an approved repository-external temporary JSON remain dated facts; that external file is no longer required for reproduction.

The repository runner reads and validates the public fixture itself, ignores caller-provided synthetic environment values, and injects the approved values only into each current probe child environment as `WRITESTORM_CODEX_SYNTHETIC_INPUT` and `WRITESTORM_CODEX_SYNTHETIC_EXPECTED`. Evidence retains only fixture hashes and identifiers, never the input or expected text. The runner removes API credential environment names, hides Electron windows, ignores raw process output and retains only sanitized summaries under the operating-system temporary directory when `WRITESTORM_CODEX_KEEP_SANITIZED_RESULTS=1` is explicitly set.

With the versioned public fixture present at its repository path, the only verdict-producing command is:

```text
npm run certify:codex:windows
```

The command requires a fully clean HEAD equal to its configured upstream, then runs the full repository check, packages the isolated certification build into a unique staging directory, validates its package boundary, runs development, lifecycle and packaged gates in order, applies the unified admission/lineage/artifact verifier, and only then atomically renames the staging directory into `out/6a/<certificationId>`. The short `b6a-<head8>-<nonce8>` identifier, `.s-` staging prefix and 110-character artifact-root budget keep the longest pinned Windows SDK runtime path below the legacy 260-character process boundary; an over-budget workspace fails before packaging. A failed stage removes only that positively identified staging directory and publishes no bundle. An existing destination is never overwritten.

The immutable bundle retains the exact Windows artifact, all seven sanitized evidence records, the mechanically derived verdict, an artifact manifest with every relative path/size/SHA-256, and a bundle manifest binding the Git HEAD, artifact aggregate SHA-256, artifact-manifest SHA-256, verdict SHA-256 and every evidence SHA-256. Ordinary `npm run build` continues to use the normal product `out` target and cannot overwrite a published certification bundle. Final remote handoff must preserve or upload the complete bundle and report its immutable locator plus `bundleManifestSha256`; a hash without retrievable bundle custody is insufficient.

`probe:codex:dev` and `probe:codex:lifecycle` remain low-level diagnostic commands only. `probe:codex:packaged` delegates to the atomic certification command. None of these commands is a product fallback or Task 13 workflow.

The runner evaluates results through an exact fail-closed contract, not a negative substring scan. Each mode freezes the complete top-level evidence envelope as well as result count, task, source, classification, evidence identity, command name, SDK version and exact assertion-key set. `prompt`, `stdout`, `stderr`, arbitrary producer fields, missing fields and every other unlisted top-level key are rejected. The permitted `limitations` array is also an exact string allowlist rather than a free-text channel. Every assertion key and its exact `{ value, source, evidenceId, classification }` leaf are frozen; legacy booleans and producer-invented provenance are rejected.

Development evidence requires exactly seven cwd/Git/auth scenarios: four isolated-auth diagnostic scenarios, current-auth explicit Git, current-auth non-Git with checking, and current-auth non-Git with `skipGitRepoCheck`; it also requires the exact valid-minimal and pinned invalid-schema scenarios. `runtime_failed / unverified` in the four isolated-auth diagnostic scenarios is retained as conditional limitations rather than hard blockers when the required core is independently proven: current-auth Git success, valid-minimal outputSchema success, pinned invalid-schema rejection, the current-auth non-Git check/skip differential, and all privacy/process/lineage boundaries. An unknown SDK/CLI failure is still only the generic safe `SDK_RUNTIME_UNAVAILABLE`; it is never upgraded to an auth, login or Git fact. Missing required-core success, an incomplete Git differential, a malformed/unknown/partial/wrong-source record, a missing or extra field, or any false required assertion remains a hard blocker and produces non-zero.

Lifecycle recertification requires exactly the four named scenarios, their distinct trigger/event records, a single cleanup execution, the timeout cleanup summary, and separate utility/CLI attribution plus residual-absence assertions. Packaged recertification requires the packaged-only gate, exact approved input and expected-value SHA-256 fingerprints, the OS-temporary/validated-UUID result-path policy, Windows x64 runtime, the complete structured result and cleanup assertions.
...
## Versioned feasibility manifest

`config/block6a-feasibility-manifest-v1.json` is the single operational authority for the pinned SDK/CLI/platform versions, public synthetic fixture path and hashes, ten ordered static evidence inputs, seven runtime evidence IDs, exact Windows runtime-file allowlist and fixed conditional verdict. TypeScript feasibility runtime and Node certification tooling load that same JSON; operational source may not repeat those values. Tests may retain human-readable version text in descriptions, while executable expectations must read the manifest.

The public fixture and committed evidence records necessarily retain their own self-describing IDs and hashes, but consistency tests require them to equal the canonical manifest. The manifest, both loaders and fixture are part of the runtime-boundary hash. Changing any manifest value requires a new manifest ID, explicit review and fresh Windows certification; old evidence remains historical and cannot certify the changed boundary.

## Feasibility harness exit contract

The machine-readable disposition authority is `docs/engineering/block6a-feasibility-harness-exit-plan.json`. It preserves the original classification of every former `src/main/codex-feasibility` file and records the completed exit to `tests/certification/block6a/runtime`. Nothing in the retained fixture is a production `AiExecutionPort` or `CodexProviderAdapter`, and product Task 13/17 code may not import it as a shortcut.

`environment.ts`, `lifecycle.ts`, `session-supervisor.ts`, `termination-supervisor.ts` and `utility-protocol-termination.ts` are `extract-then-delete`. Only their reviewed low-level allowlist, owned-process identity, single-flight, settlement and safe-termination semantics may be independently extracted behind new production types and tests. The original feasibility files must then be deleted; their probe types, classifications and evidence fields must not cross into the production contract.

All remaining feasibility source files are `delete-without-promotion`. In particular, `runner.ts`, `utility-entry.ts`, `protocol.ts`, `operations.ts`, the probe main files, synthetic structured-output validator, fixed probe deadlines, assertion provenance and certification main must never become Job/runtime code. The production targets are new `AiExecutionPort`, `CodexProviderAdapter` and Codex utility entry files under `src/main/ai`; their stable error/event/cancellation contracts require a separate Task 13 design and may not depend on raw SDK/CLI English text.

The Task 13.2 mechanical no-import guard passed before production adapter composition, and the first product AI execution exit requirement is now complete: `src/main/codex-feasibility` is absent and the retained Block 6A fixture, admission, lineage and certification tooling are outside the product source and default package graph. Before Task 17 release readiness, that replacement certification path must pass against the final artifact. This completed source exit does not implement a Job adapter, change macOS `deferred-by-user` status or make current packaged evidence fresh.
