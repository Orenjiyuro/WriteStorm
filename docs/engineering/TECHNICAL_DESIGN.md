...
The SDK root is import-only, so the production CJS utility preserves a runtime `import()` and fails closed if the expected `Codex` export is absent. Production Forge/Vite and the deterministic smoke import one shared utility build configuration; the smoke changes only the isolated entry/output and adds a test-only network-guard banner. It starts that instrumented production entry through Electron `utilityProcess.fork`, verifies the exact fixed bundle, imports the SDK without constructing a client or using auth/proxy configuration, sends one unsupported message and observes exit code 28 plus owned temporary cleanup. The guard replaces `fetch`, DNS and Node HTTP/HTTPS/TLS/socket entry points before SDK import, blocks any attempt, and persists only installed state, attempt count and a closed operation kind. Missing observation fails; a negative witness proves `net.connect` is blocked. This is build-tree evidence, not the Task 13.11 production-package runtime gate.
