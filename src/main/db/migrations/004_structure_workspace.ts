...
export const STRUCTURE_WORKSPACE_MIGRATION = {
  ...
  up(database) {
    assertLegacyStructureShellIsEmpty(database);

    database.exec(`
      DROP TABLE story_segment_ranges;
      DROP TABLE structure_nodes;
...
function assertLegacyStructureShellIsEmpty(database: Parameters<Migration['up']>[0]): void {
  const legacyTableNames = ['structure_nodes', 'story_segment_ranges'] as const;

  for (const tableName of legacyTableNames) {
    const rowCount = database.prepare(`SELECT COUNT(*) FROM ${tableName}`).pluck().get();

    if (typeof rowCount !== 'number' || rowCount !== 0) {
      throw new Error('Legacy structure shell tables must be empty before structure workspace migration.');
    }
  }
}
