...
| Local secondary-display development gate | `playwright.config.ts`, `tests/e2e/display-policy.ts`, packaged Electron specs and the explicit secondary launcher | Local development Playwright fails closed without a secondary display. CI receives no implicit local target. `npm start`, installed builds and ordinary product launches never load this test policy and do not require a secondary display. |
