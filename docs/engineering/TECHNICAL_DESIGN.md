...
The latest historical recertification evidence binds clean runtime HEAD `df7e9d20d1e2e24e04aafa62464092a39c3a8121` to compatibility fingerprint `c288a454be6e4546c321b16ec3fa6c6ac8a6f8be644fd8551d6f5ca566c374d8`. The product artifact passed isolated success, cancel and timeout, while the separate certification artifact passed all 17 no-global-Git assertions. The canonical compatibility leaf and Settings natural-path runner change all three fingerprint layers, so those records cannot authorize the current candidate. An ordinary build embeds stale compatibility. An explicit certification build may embed the current source fingerprint. Runtime accepts the external receipt only when its fingerprint, ordered file sizes/hashes, aggregate receipt hash, and streamed SHA-256 checks of the exact packaged executable, `app.asar` and pinned Codex executable all agree. Missing, malformed or mismatched receipt is stale and erases auth observation to unknown. The receipt proves byte equality only; without a signing key or trusted OS signature it is not a cryptographic proof that a probe ran or passed. Main-thread acceptance of separately authorized product, no-global-Git and Settings natural-path evidence remains the probe-provenance authority. Default packaged E2E remains offline and never clicks the real connection action.
