...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the current Codex feasibility authority. Its current verdict is `conditional Go — Windows-only feasibility verified; macOS deferred-by-user`. The historical Task 6A.8b verdict expired when R1–R7 changed admission, environment, protocol, unified utility-session orchestration, supervision, cleanup, error classification, assertion provenance and lineage; R8a5 now supplies fresh development, four-scenario lifecycle and artifact-bound Windows x64 packaged evidence for the repaired boundary. Fresh assertion leaves bind value, source, evidenceId and classification so local/static facts cannot inherit a runtime record's source. Fresh lineage binds the clean committed run HEAD, lockfile, runtime boundary, packaged artifact and ten static evidence inputs; only evidence/authority/consistency-test changes may follow a run. The conditional verdict still depends on existing ChatGPT-managed auth and generic safe failure handling where SDK 0.144.6 exposes no structured Git/auth discriminant. macOS packaged runtime remains `deferred-by-user`; this is not full Go, cross-platform compatibility or release readiness. Task 13.1 remains blocked pending total-thread review and Task 13.2 is not authorized. Historical Block 7/8 records that 6A had not run at those checkpoints also remain historical facts.
...
The fresh R8a5 run from clean HEAD `9eb679c` passed the development gate as `admitted_with_conditions` with no blockers. Current-auth Git and current-auth non-Git with `skipGitRepoCheck=true` both completed authenticated expected-result turns; the matching non-Git checked case failed only as `SDK_RUNTIME_UNAVAILABLE`, establishing the approved behavioral differential without message parsing. Valid-minimal and pinned invalid-schema outcomes both passed. Four conditional limitations remain: no structured Git/auth discriminant, isolated-auth local deadline, isolated-auth generic unavailable, and generic-only classification for the checked non-Git failure. Both records bind ten inputs and clean runtime hashes. Development now permits lifecycle recertification, but Windows remains pending until lifecycle and packaged gates independently pass.

The fresh Windows lifecycle run from clean HEAD `b29c859` passed app-timeout, explicit-cancel, window-close and app-quit with exact trigger matching, abort observation, SDK settlement, cleanup acknowledgement, utility exit and attributed zero-residual checks. Window close and app quit remain distinct initial triggers, and cleanup executed once even when closing the final window also caused quit notifications. The four records bind ten static inputs and the clean runtime boundary without persisting process identities or raw errors. Lifecycle now permits the Windows packaged gate; the Windows verdict remains pending until that independent artifact-bound run passes.

The Windows x64 packaged command from clean HEAD `e741bda` passed the package boundary and a real authenticated packaged SDK structured turn. Because the required full check subsequently rebuilt `out/`, the historical packaged-authority test was restored to its pre-run content so all earlier lineage remained valid, and the unchanged rebuilt artifact passed package verification 7/7 plus a final packaged-only turn from clean HEAD `da75638`. The current `packaged_sdk` record binds that exact artifact hash, lockfile/runtime boundary and ten static inputs. The two static privacy/protocol assertions retain their own provenance. This completes the Windows feasibility evidence chain and restores only the Windows-only conditional verdict. Task 13.1 remains blocked for total-thread review, Task 13.2 remains unauthorized, and macOS packaged runtime remains `deferred-by-user`.

The final repository check is not fully green on this branch: typecheck, 830 unit tests and 290 integration tests passed, while packaged E2E passed 14/15. The only failure is the Library-session cache case intentionally isolated under the separate `602500f` salvage decision; this 6A branch must not include that renderer fix. The package-boundary suite independently passed 7/7. This limitation is reported to the total thread and is not hidden by the Windows feasibility verdict.
