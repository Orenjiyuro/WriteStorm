...
The integration candidate at clean and pushed HEAD `2389170` includes the authorized Library-session cache fix `602500f`. Fresh `npm run check` is fully green: typecheck, 831 unit tests, 290 integration tests, packaged E2E 15/15 and package boundary 7/7. The resulting artifact is the one bound by the current packaged SDK evidence.

Fresh development recertification from that same integration HEAD passed as `admitted_with_conditions` with no blocker. Current-auth Git, the non-Git bypass differential, valid outputSchema and the pinned invalid-schema guard passed; four known Git/auth diagnostic limitations remain. Fresh lifecycle then passed app-timeout, explicit-cancel, window-close and app-quit with abort observation, SDK settlement, cleanup acknowledgement, utility exit and attributed zero-residual checks.

The packaged-only SDK turn from the same integration HEAD passed with authenticated structured success and cleanup acknowledgement. The current `packaged_sdk` record binds the exact full-check artifact hash, lockfile/runtime boundary and ten static inputs. The two static privacy/protocol assertions retain their own provenance. This completes the merge-candidate Windows feasibility evidence chain. Task 13.1 still requires total-thread review, Task 13.2 remains unauthorized, and macOS packaged runtime remains `deferred-by-user`.
