...
Every migration-owned CHECK, trigger, and partial-index predicate is also registered as a stable semantic boundary with both an accepted action and an exactly classified rejected action. The migration contract freezes the boundary inventory (001: 12 CHECKs; 002: 32 CHECKs, 10 triggers, and one partial index), and mutation gates prove that deleting, relaxing, or tightening a constraint cannot pass merely because object names and PRAGMA structure remain unchanged.
