...
describe('structure detection IPC factory', () => {
  ...
  it.each([
    {
      error: new StructureServiceError(
    ...
    {
      error: new StructureSourceSnapshotError(
        'source_hash_mismatch',
        'The imported source changed outside WriteStorm.',
      ),
      recoverable: true,
    },
    {
      error: new StructureServiceError(
        ...
        'The structure worker has already been disposed.',
      ),
      recoverable: false,
    },
    {
      error: new StructureServiceError(
        ...
        'The candidate transaction failed.',
      ),
      recoverable: false,
    },
  ])('maps $error.reason through the stable recovery taxonomy', async ({ error, recoverable }) => {
    const dependencies = createStructureDetectionIpcDependencies({
      service: {
