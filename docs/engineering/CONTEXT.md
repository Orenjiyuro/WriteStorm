...
`docs/engineering/V1-BLOCK-6A-CODEX-SDK-FEASIBILITY.md` is the current Codex feasibility authority. Its current verdict is conditional Go for Windows-only feasibility: the pinned SDK/CLI supply chain, dedicated utility boundary, isolated cwd/Git/environment/auth behavior, structured output, supervised runner timeout/cancellation cleanup and Windows x64 packaged SDK turn are verified. The real app-timeout probe now exercises cancel-active, SDK settlement, shutdown acknowledgement, utility exit and attributed CLI residual checks; a forced fallback fails closed without the same exit/residual evidence. macOS packaged runtime is `deferred-by-user`; this is not full Go, cross-platform compatibility or release readiness. The verdict does not authorize Task 13.2, which remains a total-thread review decision. Historical Block 7/8 records that 6A had not run at those checkpoints remain historical facts rather than current authority.
