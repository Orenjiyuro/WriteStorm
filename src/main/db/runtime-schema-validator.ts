...
import { runMigrations, type Migration, type SchemaSemanticWitness } from './migration-runner';
...
function validateSemanticWitnesses(
  ...
  try {
    ...
    for (const witness of witnesses) {
      const failure = runWitness(isolated, witness);
      if (failure !== null) {
        return failure;
      }
    }
...
function runWitness(database: SqliteDatabase, witness: SchemaSemanticWitness): string | null {
  const savepoint = `schema_witness_${Math.random().toString(16).slice(2)}`;
  database.exec(`SAVEPOINT ${savepoint}`);
  try {
    if (witness.setupSql) {
      database.exec(witness.setupSql);
    }
    let rejected = false;
    try {
    ...
    const passed = witness.outcome === 'reject' ? rejected : !rejected;
    return passed ? null : `semantic witness failed: ${witness.name}`;
  } finally {
