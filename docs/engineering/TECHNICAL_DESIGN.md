...
The SDK root is import-only, so the production CJS utility preserves a runtime `import()` and fails closed if the expected `Codex` export is absent. The default offline integration smoke builds the real production entry, starts it through Electron `utilityProcess.fork`, verifies the exact fixed bundle, imports the SDK without constructing a client or using auth/network configuration, sends one unsupported message and observes exit code 28 plus owned temporary cleanup. This is build-tree evidence, not the Task 13.11 production-package runtime gate.
