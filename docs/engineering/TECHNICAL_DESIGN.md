...
The product packaged-probe result writer accepts only a UUID-owned directory created atomically beneath the canonical OS temp root. It rejects an existing run path, symbolic link, junction or reparse substitution, records directory identity, revalidates canonical containment and identity immediately before an exclusive write, and never accepts a caller-provided arbitrary output path.
